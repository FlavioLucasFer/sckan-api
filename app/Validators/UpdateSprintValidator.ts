...
export default class UpdateSprintValidator {
  ...
  public schema = schema.create({
		...
		startsAt: schema.date.optional({ format: 'yyyy-MM-dd HH:mm:ss' }, [
			rules.after(this.refs.allowedDate),
		]),

		endsAt: schema.date.optional({ format: 'yyyy-MM-dd HH:mm:ss' }, [
			rules.after(this.refs.allowedDate),
		]),
